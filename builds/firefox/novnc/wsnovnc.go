...
// go env -w GOPROXY=https://goproxy.cn && go env -w GO111MODULE=auto && go build --ldflags '-w -s' wsnovnc.go && upx wsnovnc
...
func main() {
	...
	crtfile := flag.String("cert", "simp.crt", "cert pem for tls")
